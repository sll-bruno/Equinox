import pandas as pd
from src.run_baselines import COSTS_BPS, build, make_ledger, turnover


def test_complete_cycle_cost_equals_two_half_turns():
    position = pd.Series([0, 1, 1])
    trading, terminal, total = turnover(position)
    assert trading.sum() == 1
    assert terminal.sum() == 1
    assert total.sum() == 2
    assert total.sum() * COSTS_BPS["fee_only"] / 20_000 == 0.0031
...
def test_terminal_exit_does_not_change_gross_return():
    ...
    net_without_exit = ledger.gross_return - ledger.trading_turnover * COSTS_BPS["fee_only"] / 20_000
    net_with_exit = ledger.gross_return - ledger.turnover * COSTS_BPS["fee_only"] / 20_000
